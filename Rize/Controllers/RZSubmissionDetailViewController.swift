    ...
    @IBAction func redeem() {
        ...
        RZDatabase.sharedInstance().redeemCodeForChallenge(challengeId: challenge!.id!, tier: submission!.currentTier!) { (code) in
            if code != nil
            {
                ...
                self.present(alert, animated: true, completion: nil)
                RZDatabase.sharedInstance().addCodeToWallet(challengeId: self.challenge!.id!, tier: thisTier, title: self.challenge!.tiers[thisTier].title, code: code!, icon: self.challenge!.iconUrl!, challengeTitle: self.challenge!.title!, banner: self.challenge!.bannerUrl!)
            } else {
